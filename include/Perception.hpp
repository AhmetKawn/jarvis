  ...
  Şimdi src/Perception.cpp dosyasını yazalım. Burada ses kaydı için Windows'un waveIn API'sini (en alt seviye ses
  API'si) kullanacağız. Bu kodlar oldukça uzundur çünkü donanımla doğrudan konuşuruz.
  ...
  void Perception::recordAudio(const std::string& filename, int durationSeconds) {
      ...
      HWAVEIN hWaveIn = waveInOpen(&wfx, 0, 0, 0, 0, CALLBACK_FUNCTION);
      if (hWaveIn == NULL) {
